...
const electronAPI = {
  ...
  // Add click-through toggle methods
  toggleClickThrough: () => ipcRenderer.invoke("toggle-click-through"),
  isClickThroughEnabled: () => ipcRenderer.invoke("is-click-through-enabled"),
  
  onClickThroughChanged: (callback: (isEnabled: boolean) => void) => {
    const subscription = (_: any, isEnabled: boolean) => callback(isEnabled)
