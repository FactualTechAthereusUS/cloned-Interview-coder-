...
export class ConfigHelper extends EventEmitter {
  ...
  // Add a memory fallback for configs
  private _memoryConfig: Config | null = null;

  /**
   * Load configuration from disk or memory fallback
   */
  public loadConfig(): Config {
    try {
      // If we have a memory-only config, use that
      if (this._memoryConfig) {
      ...
      // If no config exists or we couldn't read it, create a default one
      const defaultConfig = { ...this.defaultConfig };
      
      // Try to save, but handle quietly if it fails
      try {
        this.saveConfig(defaultConfig);
      } catch (saveErr) {
        console.warn("Could not save default config, using memory-only:", saveErr);
        this._memoryConfig = defaultConfig;
      }
      ...
      this._memoryConfig = fallbackConfig;
  ...
  public saveConfig(config: Config): void {
    try {
      // First verify directory permissions before attempting to write
      const configDir = path.dirname(this.configPath);
      let canWrite = false;
      
      try {
        // Try to ensure the directory exists with proper permissions
      if (!fs.existsSync(configDir)) {
          fs.mkdirSync(configDir, { recursive: true, mode: 0o755 });
          console.log(`Created config directory: ${configDir}`);
        }
        
        // Test write permissions by writing to a temporary file
        const testPath = path.join(configDir, '.test_write_permissions');
        fs.writeFileSync(testPath, 'test', { mode: 0o644 });
        fs.unlinkSync(testPath);
        canWrite = true;
      } catch (permErr) {
        console.error(`Permission error for directory ${configDir}:`, permErr);
        
        // Try alternative locations if we can't write to the preferred location
        try {
          // Try user's home directory
          const homeDir = app.getPath('home');
          const altConfigDir = path.join(homeDir, '.interview-coder');
          
          if (!fs.existsSync(altConfigDir)) {
            fs.mkdirSync(altConfigDir, { recursive: true, mode: 0o755 });
          }
          
          // Test write permissions
          const testPath = path.join(altConfigDir, '.test_write_permissions');
          fs.writeFileSync(testPath, 'test', { mode: 0o644 });
          fs.unlinkSync(testPath);
          
          // Update config path to use this alternate location
          this.configPath = path.join(altConfigDir, 'config.json');
          console.log(`Using alternative config path: ${this.configPath}`);
          canWrite = true;
        } catch (altErr) {
          console.error('Could not use alternative config location:', altErr);
          throw new Error(`Cannot find writable location for config: ${altErr.message}`);
        }
      }
      
      if (!canWrite) {
        throw new Error('No writable location found for configuration');
      }
      
      // Write the config file with try-catch
      try {
        const tempPath = `${this.configPath}.tmp`;
        
        // Write to temporary file first
        fs.writeFileSync(tempPath, JSON.stringify(config, null, 2), { mode: 0o644 });
        
        // Rename temp file to actual config file (this is more atomic)
        fs.renameSync(tempPath, this.configPath);
        
        console.log(`Config saved successfully to ${this.configPath}`);
      } catch (writeErr) {
        console.error(`Error writing config file to ${this.configPath}:`, writeErr);
        
        // Last-resort attempt: try to store in memory only
        console.log('Falling back to memory-only configuration');
        
        // Store in memory but don't persist to disk
        this._memoryConfig = config;
        
        // We won't throw here to allow app to continue running
      }
    } catch (err) {
