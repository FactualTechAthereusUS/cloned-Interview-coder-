...
export function initializeIpcHandlers(deps: IIpcHandlerDeps): void {
  ...
  ipcMain.handle("toggle-click-through", () => {
    console.log("toggle-click-through ipc handler called");
    deps.toggleClickThrough();
    
    // Send event back to renderer to update UI
    const mainWindow = deps.getMainWindow();
    if (mainWindow) {
      const isEnabled = deps.isClickThroughEnabled();
      mainWindow.webContents.send("click-through-changed", isEnabled);
    }
    
    return { success: true, isEnabled: deps.isClickThroughEnabled() };
  });
  
  ipcMain.handle("is-click-through-enabled", () => {
    return deps.isClickThroughEnabled();
  });
}
